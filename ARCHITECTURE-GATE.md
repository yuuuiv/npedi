...
`voyage/vesselList`。2026-07-29 的一次探测返回 **14,337 条航次记录
（13,954 个船×航次对，2,148 艘船）**；7 月 29/30 日快照刷新完成后共落库
**14,315 个船×航次对**，且每一对的 GATE_IN、GATE_OUT 两个方向均已完成。
历史小批补入 3 对后，当前 `gate_voyages=14,318` 且均双向完成。这里的“完成”
只表示已进入目录的航次完成，不表示完整 CODECO 历史或全港覆盖。两份目录在最初
探测时的关系如下：

| 对比 | 数量 |
|---|---|
| npp 目录（=现库 `voyages`） | 848 对 |
| gate 目录 `vesselList`（最初探测） | 13,954 对 |
...
**正确结论：该 `vesselList` 快照比当时的 npp 在册目录更宽，能补到一部分 npp
已下架航次，但它不是历史船×航次全集，不能据此声称补全了 CODECO 历史。**

**单点数据保留下界（2026-07-29 实测）：至少存在 2022 年 1 月的记录。** 这只证明
个别旧航次仍可查询，不证明从 2022 年起连续或完整覆盖。铁路驳船系列的航次号是
日期编码（`220110A` = 2022-01-10），定向探测结果：
...
### 0.1 覆盖审计：目录回填完成不等于历史全量

用计划事实表的可信窗口 **2023-01 至 2026-07** 重新核对后，共有 **292,871 个
船×航次**；其中补爬前快照目录只命中 **9,476 个**，缺少 **283,395 个**。快照
`gate_voyages=14,315` 且全部双向完成，所以这个差额不是断点未续跑或回填任务没
跑完，而是 2026-07-29/30 `vesselList` 快照本身没有给出绝大多数历史候选航次。
首次小批补入 3 对后，当前命中 9,479、仍缺 283,392。

| 月份 | 补爬前快照目录命中 / 计划事实表船×航次 |
|---|---:|
| 2025-07 | 2 / 6,775 |
| 2026-01 | 4 / 7,716 |
| 2026-06 | 316 / 7,306 |
...
覆盖在 2026 年 7 月骤然集中，因此旧月份与 2026 年 7 月的 CODECO 数量和曲线
**不可直接比较**；该形状首先反映候选目录覆盖差异，不能解释成闸口业务量或吞吐量
突然增长。

2026-08-13 使用新账号恢复认证后，对目录外 **21 个跨月船×航次样本**进行点查，
命中 **3 个**：2026-05 两个、2026-06 一个，合计返回 **1,012 条 CODECO**。
这已经证实 `vesselList` 会遗漏接口仍可查询的航次，不能用目录回填完成来解释历史
缺口。此次早期月份样本暂时为空，但样本量不足，既不能证明早期没有数据，也不能
确定服务端留存下界。

现已从船期历史生成独立候选表，并提供 1–4 worker 的小批断点补爬。2026-05/06
首次 5 对验证实际探测 4 对、全部命中：3 对完整入库 691 条，另 1 对在达到 15 次
请求预算时保存为命中待续。该定向小样本只验证链路，不可外推总体命中率。

默认命令每批 10 对、50 次请求（含认证）、请求间隔 1–1.25 秒，并优先当前 CODECO
目录已知船码。未知船码是后续显式阶段；在两阶段和分月覆盖验收完成前，旧趋势只能
视为已观测样本，仪表板不绘制 CODECO 跨期折线。

目标：

1. 以 2026-07-29/30 `vesselList` 快照的 14k 航次为入口，把该目录范围内的进门
   （GATE_IN）/出门（GATE_OUT）记录回填入库（多日断点续爬）。
...
### 4.0 `vesselList` 目录外历史补爬（进行中）

```powershell
python sync.py gate-history-backfill `
  --eta-start 2026-05-01 --eta-end 2026-06-30
```

- 候选来自 `fact_vessel_plan_snapshot`，主表记录船×航次状态，月度证据表避免跨月
  航次被错误算进中间月份；同一范围生成后会复用，不在每个小批次重扫事实表。
- 单进程持有 gate 锁，内部可用 1–4 个 worker；候选先去重再分片，每个 worker
  使用独立 HTTP/SQLite 连接。默认仍为 1 worker、每批 10 对、50 次请求。
- 双方向第一页都为 0 才记 `empty`；命中先记 `hit`，预算不足则保留待续。
- 命中后提升到 `gate_voyages`，沿用双方向完成断点；进程中断后只补未完成方向。
- 默认只探测已知船码以控制初始请求量；`--include-new-vessels` 是后续完整性阶段，
  因此已知船阶段完成不等于历史全量。

### 4.1 快照目录范围全量回填（一次性，多日任务）

本节“全量”始终指对 2026-07-29/30 `vesselList` 快照枚举出的船×航次逐一查询
两个方向。快照目录的 **14,315 对已全部双向完成**；小批历史补入后当前表为
14,318 对且均双向完成。它不代表计划事实表中的 292,871 个历史船×航次，也不
代表全港 CODECO 历史。
...
对已经被 gate 目录枚举出的旧航次，接口报文通常只增不改；日常增量主要巡查活跃
航次。目录外仍有可查询记录已由 2026-08-13 点查证实，不能把“未被枚举”当成
“没有数据”；目录外候选需要从船期历史补充并分层补爬。
...
1. `gate_events_all.csv` — 当前 gate 目录范围内全部已采集记录的快照，增量轮覆盖重写。
