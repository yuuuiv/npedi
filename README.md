...
| 进出门 | 码头进门/出门 CODECO 报文流水     | 计划事实衍生的候选，2022–2026 已补齐、2020–2021 进行中，运行 `gate-status` 查看 | `gate-*`                   |
...
## vesselList 快照目录内的箱号、闸口历史与远程增强

箱号候选不再依赖只有 53 行的旧增强队列。`seed-container-catalog` 从已经回填完成的 `gate_events` 建立目录，并分别保存 VGM 与单箱轨迹的状态。当前主库已经建立 2,488,395 个箱号，其中 2,488,083 个通过 ISO 6346 校验；无效箱号保留用于审计，但不会发送到只能按箱号过滤的接口。这些数量的全集边界是已采集的 gate 快照目录，不是全港历史箱号全集。
...
`container_event_full` 统一视图直接读取当前 96,258,674 条 CODECO 源记录，提供已采集范围的 `IN_GATE` / `OUT_GATE` 历史，不再把上亿事件复制一份到事实表。这里的 `full` 只表示已采集源记录全部纳入，既不是完整 CODECO 历史，也不是官方统计口径的全港吞吐量。`observed_at` 使用报文采集时间，所以严格 as-of 回测不会在“当时尚未采集”时提前看到历史事件。

### vesselList 快照不是全量目录，历史靠计划事实补

`vesselList` 是个目录快照，只有 14,315 个船×航次；而计划事实表在 2023-01 至
2026-07 这个可信窗口里有 292,871 个。差额不是断点续跑失败，是快照本身就没有
那些航次。所以真正的历史候选是**从计划事实反推**出来的，攒进
`gate_history_candidate` 队列后逐个点查，共 474,018 个。

按 `last_eta` 从新往旧探测，当前进度：

| ETA 年份 | 待探测 | 候选总数 | 完成度 |
|---|---:|---:|---:|
| 2022–2026 | 255 | 358,848 | 99.9% |
| 2021 | 27,418 | 57,345 | 52.2% |
| 2020 | 57,825 | 57,825 | 0% |
| 合计 | **85,498** | 474,018 | **82.0%** |

回填后 `gate_voyages` 从 14,318 涨到 **225,877**，`gate_events` 从 366 万涨到
**9,626 万**。

**2020–2021 补齐之前，跨年份的 CODECO 曲线不能当趋势看** —— 那两年的低值是
没采到，不是业务量低。

续跑用包装脚本，它管分 chunk、断点、限速和 401 重启：
...
只起一个 CLI 进程，worker 由它内部调度，别在后台叠多个。候选按日期范围缓存，
续跑不会反复扫主库。

**最可能打断你的还是 token。** 实测服务端按请求量掐会话，近期阈值降到 4–6 万次
请求（6 workers 下 1–3.5 小时），失效后需要人工换 `WEB_TOKEN`。详见
[HANDOVER.md](HANDOVER.md)。

### 虚拟船和运营聚合桶单独存库

接口里混着中转、空返、堆场倒箱、吊机检修这类非物理航次，报文量极大——实测
`UN7654321`（中转梅山）单个航次返回 163 万行。这些键记进
`gate_history_rejected_pair`（3,555 条）并从正常读取路径排除，报文本体由
`gate_anomaly_sidecar.py` 存到独立的 `gate_anomaly_sidecar.sqlite`，
不混进 `gate_events`。判定按精确的船×航次对生效，不会因为一个船码可疑就全局封杀。
详见 [docs/GATE-ANOMALY-SIDECAR.md](docs/GATE-ANOMALY-SIDECAR.md)。
...
站点的**进出门查询**挂的是另一份目录。最初靠 2026-07-29/30 的 `vesselList` 快照
拿到 14,315 个船×航次对；后来发现快照远不是全集，改成从计划事实反推候选逐个点查，
当前 `gate_voyages` 已经到 **225,877**，覆盖到 2021 年 7 月初，还在往 2020 年补。
相对只有 848 个在册航次的 npp 目录，这条线补回了大量已下架的历史航次。
...
回填要跑十几个小时，建议放在一台常开的机器上，见[在远端机器上跑 vesselList 快照目录回填](#在远端机器上跑-vessellist-快照目录回填)。

**这是个大活。** `gate-backfill` 跑的是快照目录那 14k 个航次 × 2 个方向，六到十二万次请求、十几到二十几小时。补历史用的是另一条命令（`gate-history-backfill`，见上面那节），量级完全不同：47.4 万个候选，至今已跑了六天。请求数会随航次变大而下降 —— 满载的远洋船单个航次能有近 2000 条报文（20 页）。
...
python sync.py gate-status
  航次目录   : 225877
  历史候选   : 474018（已探测 388520 / 82.0%，待探测 85498）
```
...
| 文件                     | 内容                                             | 更新方式                       |
| ------------------------ | ------------------------------------------------ | ------------------------------ |
| `gate_events_all.csv`    | gate 目录范围内全部已采集进出门报文的当前态      | 增量轮覆盖重写，回填轮默认跳过 |
...
`gate_gap_vs_npp.csv` 是快照目录范围内补数效果的验收表。它按箱号、提单号、UN 号、航次四列关联两边，并且已经滤掉了 npp 的双码头空壳行（见下一节），所以缺口数不会虚高。里面绝大多数应该是 npp 已下架的历史航次；但这张表不能发现从未进入 `vesselList` 快照的船×航次。

## 在远端机器上跑 vesselList 快照目录回填
...
container-history 确认只能逐箱查询。需要续跑时使用带原子队列认领的受控 worker；
默认和建议值均为 **1 个 worker**，除非另行做过限流验证，不要主动加并发：
